...
def preprocess_slot_entrie(projects, all_time_entries, start_date, end_date):
    # Convierte la lista de entradas de tiempo en DataFrame
    merged_df = process_data(projects, all_time_entries, start_date, end_date)

    # Agrega un contador
    slot_entrie_df['counter'] = range(1, len(slot_entrie_df) + 1)
    ...
    stop_diff = slot_entrie_df['stop'].diff().dt.total_seconds() / 3600  # convierte la diferencia a horas
    slot_entrie_df['excess_time'] = slot_entrie_df['duration'] - stop_diff

    # Calcula la diferencia de tiempo con la entrada anterior
    slot_entrie_df['time_diff'] = slot_entrie_df['start'].diff()
